...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        # self.prediction_bce = Seq(nn.Conv2d(channels[-1], 1024, 1, bias=True),
        #                       nn.BatchNorm2d(1024),
        #                       act_layer(act),
        #                       )
        self.model_init()
    ...
    def forward(self, inputs, proxies=None, length_threshold=70, m=3, alpha=0.01):
        ...
        if self.bce:
            x = self.prediction_bce(x).squeeze(-1).squeeze(-1)
